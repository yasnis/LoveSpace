...
void ofApp::setup() {
	...
	referenceImage.resize(referenceImage.getWidth() /5, referenceImage.getHeight() /5);
...
void ofApp::saveXyzMap() {
	ofFloatPixels pix;
	fboPositions.getTextureReference(0).readToPixels(pix);
...
void ofApp::drawOverlay() {
	...
	if(ofGetKeyPressed('a')) {  //TODO:fboPositionsを事前に生成しておかなければいけない
		fboPositions.begin();
		ofClear(0);
		glPushAttrib(GL_ALL_ATTRIB_BITS);
		glPushMatrix();
